...
const displayMovements = function (movements, sort = false) {
  containerMovements.innerHTML = ''; //setting default html data to empty string
...
btnTransfer.addEventListener('click', function (e) {
  e.preventDefault(); //prevent form from reloading a page
...
const checkDogs = function (dogsJulia, dogsKate) {
  // const newDogsJulia = dogsJulia.slice(1, -2); //right but not appreciated

  // Creating a shallow copy of an array
  const newDogsJulia = dogsJulia.slice();

  // Removing wrong data
  newDogsJulia.splice(0, 1); // splice is preferred to be used instead of slice
...
// flatMap - goes only one level down
...
// put all elements of dog.owners arrays into one array and chain them with 'and'

console.log(`${ownersEatTooMuch.join(' and ')}'s dogs eat too much food!`);
console.log(`${ownersEatTooLittle.join(' and ')}'s dogs eat too little food!`);

//5.
// console.log(dogs.find(dog => dog.curFood === dog.recFood) || 'false');
...
//8.
const copyDogs = dogs.slice().sort((dogA, dogB) => dogA.recFood - dogB.recFood);
